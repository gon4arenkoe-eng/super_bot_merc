...
Render-ready: no pandas/numpy heavy deps
"""
...
# PostgreSQL — optional, fallback to SQLite if not available
try:
    import psycopg2
...
def init_db():
    ...
    """) if not USE_POSTGRES else cur.execute("""
        CREATE TABLE IF NOT EXISTS users (
    ...
    cur.close()
    conn.close()
    logger.info("Database initialized successfully (%s)", "PostgreSQL" if USE_POSTGRES else "SQLite")
...
def load_user(user_id):
    ...
    if USE_POSTGRES:
        ...
        cur = conn.cursor()
    cur.execute("SELECT * FROM users WHERE id = ?" if not USE_POSTGRES else "SELECT * FROM users WHERE id = %s", (user_id,))
    user = cur.fetchone()
    cur.close()
    conn.close()
    if user:
...
class ExchangeAPI:
    ...
    def _bingx_get_positions(self):
        ...
        resp = self.session.get(f"{self.base_urls['bingx']}{endpoint}", headers=headers, params=params, timeout=10)
        data = resp.json()
        positions = []
        if data.get('code') == 0:
    ...
    def _binance_get_positions(self):
        ...
        resp = self.session.get(f"{self.base_urls['binance']}{endpoint}", headers=headers, params=params, timeout=10)
        data = resp.json()
        positions = []
        for pos in data:
    ...
    def _bybit_get_positions(self):
        ...
        resp = self.session.get(f"{self.base_urls['bybit']}{endpoint}", headers=headers, params=params, timeout=10)
        data = resp.json()
        positions = []
    ...
    def _okx_get_positions(self):
        ...
        resp = self.session.get(f"{self.base_urls['okx']}{endpoint}", headers=headers, timeout=10)
        data = resp.json()
        positions = []
        if data.get('code') == '0':
            for pos in data.get('data', []):
...
def login():
    if request.method == 'POST':
        ...
        conn = get_db()
        cur = conn.cursor()
        cur.execute("SELECT * FROM users WHERE username = ?" if not USE_POSTGRES else "SELECT * FROM users WHERE username = %s", (username,))
        user = cur.fetchone()
        cur.close()
        conn.close()
        if user and check_password_hash(user['password_hash'], password):
            ...
            login_user(user_obj)
            return redirect(url_for('dashboard'))
        return render_template('dashboard.html', error='Invalid credentials', now=datetime.utcnow())
    return render_template('dashboard.html', now=datetime.utcnow())
...
def register():
    ...
    try:
        ...
        for key, value in bot_engine.settings.items():
            if USE_POSTGRES:
                cur.execute("INSERT INTO bot_settings (user_id, setting_key, setting_value) VALUES (%s, %s, %s)", (user_id, key, json.dumps(value)))
            else:
                cur.execute("INSERT INTO bot_settings (user_id, setting_key, setting_value) VALUES (?, ?, ?)", (user_id, key, json.dumps(value)))
        conn.commit()
...
def get_exchanges():
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT id, name, is_demo, is_active, created_at FROM exchanges WHERE user_id = ?" if not USE_POSTGRES else "SELECT id, name, is_demo, is_active, created_at FROM exchanges WHERE user_id = %s", (current_user.id,))
    rows = cur.fetchall()
    cur.close()
    conn.close()
    exchanges = []
...
def add_exchange():
    ...
    is_demo = 1 if str(data.get('is_demo', 'true')).lower() in ('true', '1', 'yes', 'on') else 0
    ...
    cur = conn.cursor()
    if USE_POSTGRES:
        cur.execute("INSERT INTO exchanges (user_id, name, api_key_encrypted, api_secret_encrypted, passphrase_encrypted, is_demo) VALUES (%s, %s, %s, %s, %s, %s) RETURNING id",
            (current_user.id, name, encrypt(api_key), encrypt(api_secret), encrypt(passphrase) if passphrase else None, is_demo))
        exchange_id = cur.fetchone()[0]
    else:
        cur.execute("INSERT INTO exchanges (user_id, name, api_key_encrypted, api_secret_encrypted, passphrase_encrypted, is_demo) VALUES (?, ?, ?, ?, ?, ?)",
            (current_user.id, name, encrypt(api_key), encrypt(api_secret), encrypt(passphrase) if passphrase else None, is_demo))
...
def delete_exchange(exchange_id):
    conn = get_db()
    cur = conn.cursor()
    cur.execute("DELETE FROM exchanges WHERE id = ? AND user_id = ?" if not USE_POSTGRES else "DELETE FROM exchanges WHERE id = %s AND user_id = %s", (exchange_id, current_user.id))
    conn.commit()
    cur.close()
    conn.close()
...
def get_positions():
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT * FROM positions WHERE user_id = ? AND status = 'OPEN' ORDER BY created_at DESC" if not USE_POSTGRES else "SELECT * FROM positions WHERE user_id = %s AND status = 'OPEN' ORDER BY created_at DESC", (current_user.id,))
    rows = cur.fetchall()
    cur.close()
    conn.close()
    positions = []
    for row in rows:
        positions.append({k: row[k] for k in row.keys()})
...
def get_live_positions():
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT * FROM exchanges WHERE user_id = ? AND is_active = 1" if not USE_POSTGRES else "SELECT * FROM exchanges WHERE user_id = %s AND is_active = TRUE", (current_user.id,))
...
def close_order():
    ...
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT * FROM exchanges WHERE id = ? AND user_id = ?" if not USE_POSTGRES else "SELECT * FROM exchanges WHERE id = %s AND user_id = %s", (exchange_id, current_user.id))
    ex = cur.fetchone()
    cur.close()
    conn.close()
    if not ex:
...
def get_settings():
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT setting_key, setting_value FROM bot_settings WHERE user_id = ?" if not USE_POSTGRES else "SELECT setting_key, setting_value FROM bot_settings WHERE user_id = %s", (current_user.id,))
    rows = cur.fetchall()
    cur.close()
    conn.close()
    settings = {}
...
def update_settings():
    ...
    for key, value in data.items():
        if USE_POSTGRES:
            cur.execute("""INSERT INTO bot_settings (user_id, setting_key, setting_value) VALUES (%s, %s, %s)
                ON CONFLICT (user_id, setting_key) DO UPDATE SET setting_value = EXCLUDED.setting_value""",
                (current_user.id, key, json.dumps(value)))
        else:
            cur.execute("INSERT OR REPLACE INTO bot_settings (user_id, setting_key, setting_value) VALUES (?, ?, ?)",
                (current_user.id, key, json.dumps(value)))
    conn.commit()
    cur.close()
    conn.close()
...
def get_trade_history():
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT * FROM trades WHERE user_id = ? ORDER BY closed_at DESC LIMIT 100" if not USE_POSTGRES else "SELECT * FROM trades WHERE user_id = %s ORDER BY closed_at DESC LIMIT 100", (current_user.id,))
    rows = cur.fetchall()
    cur.close()
    conn.close()
    trades = []
    for row in rows:
        trades.append({k: row[k] for k in row.keys()})
...
def get_daily_pnl():
    ...
    cur = conn.cursor()
    if USE_POSTGRES:
        cur.execute("""SELECT DATE(closed_at) as date, SUM(pnl) as daily_pnl, COUNT(*) as trade_count
            FROM trades WHERE user_id = %s AND closed_at >= NOW() - INTERVAL '30 days'
            GROUP BY DATE(closed_at) ORDER BY date DESC""", (current_user.id,))
    else:
        cur.execute("""SELECT date(closed_at) as date, SUM(pnl) as daily_pnl, COUNT(*) as trade_count
            FROM trades WHERE user_id = ? AND closed_at >= date('now', '-30 days')
            GROUP BY date(closed_at) ORDER BY date DESC""", (current_user.id,))
    rows = cur.fetchall()
    cur.close()
    conn.close()
    pnl_data = []
    for row in rows:
        pnl_data.append({k: row[k] for k in row.keys()})
