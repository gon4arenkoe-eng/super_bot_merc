"""SUPERBOT v5.6.0 - EMA Cross Strategy (pure functions, FIXED SMA seeding)"""
from typing import Dict, List


def calculate_sma(prices: List[float], period: int) -> float:
    """Calculate Simple Moving Average. Pure function."""
...
def calculate_ema(prices: List[float], period: int) -> List[float]:
    """Calculate EMA for a list of prices. Pure function. FIXED: SMA seeding.

    Standard EMA calculation:
    1. First value = SMA of first 'period' prices
    2. Each subsequent value = (price - previous_ema) * multiplier + previous_ema
    """
    if len(prices) < period:
        return []
    ...
    # FIX: Seed EMA with SMA of first 'period' prices, not just first price
    sma_seed = calculate_sma(prices[:period], period)
    ema = [sma_seed]
